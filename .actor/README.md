# 🧩 Shopify Tech Stack Detector + Product Scraper — Apps, Reviews & Catalog

**Paste a Shopify store URL, click Run, and get the full tech stack + catalog + reviews in seconds.**

🧩 **Shopify Tech Stack Detector** is the only Apify Actor that auto-detects the **150+ apps installed on any Shopify store** (Klaviyo, Recharge, Yotpo, Judge.me, Privy, Gorgias, Algolia, Okendo, Iterable, Bloomreach, and many more) — combined with the **full product catalog** via the official `/products.json` endpoint and **reviews** from every major review provider.
...
🧩 Shopify Tech Stack Detector turns any Shopify URL into structured intelligence in **under 30 seconds**.
...
* 🧩 **Detected apps** — Klaviyo, Recharge, Yotpo, Judge.me, Privy, Gorgias, Algolia, Okendo, Iterable, Bloomreach, Braze, Smile.io, LoyaltyLion, Weglot, Rebuy, Shogun, PageFly, Hotjar, GTM, FB Pixel, TikTok Pixel, and 30+ more
...
Setting up 🧩 Shopify Tech Stack Detector takes **less than a minute**:
...
> 💡 Set `use_residential_proxy: true` only when you scrape Cloudflare-protected stores like Tesla Shop or Manscaped. Datacenter proxy is enough for 98% of Shopify stores.
...
🧩 Shopify Tech Stack Detector uses **pay-per-event** pricing. You only pay for what you actually extract:
...
Learn more about [Apify pricing](/pricing).

---

## 📦 Output format

The actor writes to **four datasets** that you can export independently:
...
* 🛡️ **Hit a 403?** Toggle `Use Residential Proxy` for Cloudflare-protected stores
* 🔄 **Schedule weekly runs** with [Apify Scheduler](/schedules) to track product changes and price drops over time
* 🪝 **Push to Sheets, Slack, Airtable** via [Apify integrations](/integrations) — no glue code needed

---

## 🌍 Supported Shopify markets and stores

🧩 Shopify Tech Stack Detector works on **any Shopify storefront worldwide** — custom domain, `*.myshopify.com`, regional subdomains, Shopify Plus.
...
## 🧭 When to use Shopify Tech Stack Detector vs alternatives

Use **🧩 Shopify Tech Stack Detector** if you want:
...
* [🧩 Shopify Tech Stack Detector + Product Scraper](/kazkn/shopify-scraper-apps-spy) — this actor
* [Vinted Smart Scraper — Cross-Country Price Comparison](/kazkn/vinted-smart-scraper) — full Vinted intelligence across 26 markets
* [⚡ Vinted Turbo Scraper](/kazkn/vinted-turbo-scraper) — fastest Vinted URL-to-dataset workflow
* [Vinted MCP Server](/kazkn/vinted-mcp-server) — connect any AI agent (Claude, Cursor, Windsurf) to Vinted data
* [App Store Scraper for Localization Gaps](/kazkn/apple-app-store-localization-scraper) — find US apps missing French, German, Spanish localizations
* [GPT Crawler MCP](/kazkn/gpt-crawler-mcp) — turn any website into a clean knowledge file for ChatGPT, Claude, RAG
...
Yes. The `/products.json` endpoint is a public, official Shopify feature documented by Shopify and consumed by Google Shopping for indexing. The actor does not bypass authentication, does not interact with Shopify's Admin API, and does not collect any buyer personal data.
...
Yes — schedule runs in Apify Console with any cadence (hourly, daily, weekly). Export to webhook, Sheets, S3, BigQuery, or any Apify integration.
...
256–1024 MB tier. Real-world use is sub-300 MB even for 1,000-product stores.
...
Generic scrapers extract one product at a time, do not understand Shopify's variants/collections/inventory model, and can't detect installed apps. This actor is purpose-built for Shopify with regex patterns tuned for the apps detection use case.
...
Note that personal data is protected by GDPR in the EU and other regulations worldwide. Do not scrape personal data unless you have a legitimate reason. If unsure, consult your lawyers. Suggested reading: [Is web scraping legal?](https://blog.apify.com/is-web-scraping-legal/).
